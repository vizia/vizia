        ...
        .build(cx, move |cx| {
            Element::new(cx)
                ...
                .bind(value, move |handle, value| {
                    let value = value.get(handle.cx).unwrap_or_default();
                    match orientation {
                ...
                .bind(ratio, move |handle, ratio| {
                    let ratio = ratio.get(handle.cx).unwrap_or_default();
                    match orientation {
            ...
            match window_event {
                WindowEvent::MouseDown(MouseButton::Left) => {
                    if meta.target != cx.current() {
                        self.reference_points = Some((pos, self.value.get(cx).unwrap_or_default()));
                        cx.capture();
                    } else {
                        ...
                        let changed = self.compute_new_value(
                            cx,
                            physical_delta,
                            self.value.get(cx).unwrap_or_default(),
                        );
