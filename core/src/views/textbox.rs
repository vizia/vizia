...
use crate::{
    Binding, Color, Context, CursorIcon, EditableText, Element, Entity, Event, FontOrId, Handle,
    Lens, Model, Modifiers, MouseButton, Movement, PropSet, Selection, Units::*, View, WindowEvent, Data, Visibility,
};
        ...
        .build2(cx, move |cx| {
            ...
            Binding::new(cx, lens.clone(), |cx, text| {
                ...
                Binding::new(cx, TextboxData::root, move |cx, text_data| {
                    ...
                    let (select_left, select_width, caret_left) = update_caret(cx, text.get(cx).as_str(), &text_data.get(cx).selection);
                ...
                if !text_data.selection.is_caret() {
                    ...
                    match movement {
                        Movement::Grapheme(Direction::Upstream) => {
                            if let Some(offset) = text.prev_grapheme_offset(text_data.selection.active) {
                                //self.text.replace_range(offset..self.selection.active, "");
                                if let Some(callback) = self.on_edit.take() {
                                    (callback)(cx, offset..text_data.selection.active, String::new());
                                    self.on_edit = Some(callback);
                                }
                        ...
                        Movement::Grapheme(Direction::Downstream) => {
                            if let Some(offset) = text.next_grapheme_offset(text_data.selection.active) {
                                //self.text.replace_range(self.selection.active..offset, "");
                                if let Some(callback) = self.on_edit.take() {
                                    (callback)(cx, text_data.selection.active..offset, String::new());
                                    self.on_edit = Some(callback);
                                }
    ...
    pub fn move_cursor(&mut self, cx: &mut Context, movement: Movement, selection: bool) {
        if let Some(text_data) = cx.data::<TextboxData>().cloned() {
            if let Some(text) = self.get_text(cx) {
                match movement {
                    Movement::Grapheme(Direction::Upstream) => {
                        let active = if let Some(offset) = text.prev_grapheme_offset(text_data.selection.active) {
                    ...
                    Movement::Grapheme(Direction::Downstream) => {
                        let active = if let Some(offset) = text.next_grapheme_offset(text_data.selection.active) {
                            //text_data.selection.active = offset;
                            cx.emit(TextEvent::SetActive(offset));
                            offset
                        } else {
                    ...
                    Movement::Word(Direction::Upstream) => {
                        let active = if let Some(offset) = text.prev_word_offset(text_data.selection.active) {
                            //text_data.selection.active = offset;
                            cx.emit(TextEvent::SetActive(offset));
                            offset
                        } else {
                    ...
                    Movement::Word(Direction::Downstream) => {
                        let active = if let Some(offset) = text.next_word_offset(text_data.selection.active) {
                            //text_data.selection.active = offset;
                            cx.emit(TextEvent::SetActive(offset));
                            offset
                        } else {
