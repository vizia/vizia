...
impl TodoApp {
    fn new(cx: &mut Context) -> Handle<'_, Self> {
        ...
        let text = cx.state(String::new());
        let filter = cx.state(TodoFilter::All);
        let todos_signal = cx.state(todos);

        // Derived signals
        let items_left = cx.derived({
            let todos = todos_signal;
            move |s| {
                let count = todos.get(s).iter().filter(|t| !t.completed).count();
                format!("{} items left!", count)
            }
        });

        let filtered_indices = cx.derived({
            let todos = todos_signal;
            let filter = filter;
            move |s| {
                let todos_list = todos.get(s);
        ...
        Self { text, filter, todos: todos_signal }.build(cx, move |cx| {
            Label::static_text(cx, "todos").class("todo-header_title");
            ...
            VStack::new(cx, move |cx| {
                ...
                HStack::new(cx, move |cx| {
                    ...
                    let is_all = cx.derived({
                        let filter = filter;
                        move |s| *filter.get(s) == TodoFilter::All
                    });
                    let is_active = cx.derived({
                        let filter = filter;
                        move |s| *filter.get(s) == TodoFilter::Active
                    });
                    let is_completed = cx.derived({
                        let filter = filter;
                        move |s| *filter.get(s) == TodoFilter::Completed
                    });

                    ButtonGroup::new(cx, move |cx| {
                        ToggleButton::new(cx, is_all, move |cx| Label::static_text(cx, "All"))
                            .on_press(|cx| cx.emit(TodoEvent::SetFilter(TodoFilter::All)))
                            .class("todo-footer_button");

                        ToggleButton::new(cx, is_active, move |cx| {
                            Label::static_text(cx, "Active")
                        })
                        .on_press(|cx| cx.emit(TodoEvent::SetFilter(TodoFilter::Active)))
                        .class("todo-footer_button");

                        ToggleButton::new(cx, is_completed, move |cx| {
                            Label::static_text(cx, "Completed")
                        })
                        .on_press(|cx| cx.emit(TodoEvent::SetFilter(TodoFilter::Completed)))
                        .class("todo-footer_button");
                    });

                    Button::new(cx, |cx| Label::static_text(cx, "Clear Completed"))
...
fn main() -> Result<(), ApplicationError> {
    let (app, title) = Application::new_with_state(|cx| {
        let title = cx.state("TodoMVC".to_string());
        TodoApp::new(cx);
        title
    });
