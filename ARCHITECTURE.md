...
This document aims to explain the architecture and structure of the Vizia codebase.

## Crates

Vizia is split into a number of internal crates for specific purposes:
...
## Overview
At the core of Vizia is a very simple ECS model. Views are assigned an Entity ID, which is used to get and set view properties (components), and a series of systems update these properties and draw the views to the window.
...
### Context
The `Context` is where all retained application state lives. This includes model data, views, style properties, mouse state, and the event queue.

### View
The `View` trait describes a visual element in a Vizia application. It has four methods:
...
### Handle
A `Handle` is a wrapper around an `Entity` and a mutable reference to the `Context`, and is returned by the `build()` method on the `View` trait. For example, it is returned when a view is built with `Label::new(cx, "Hello Vizia")`.

### Modifiers
The `Handle` implements a number of `Modifiers`, which are traits that provide methods for setting the properties of a view at build time. For example, the `StyleModifiers` trait provides methods for setting the style properties of a view such as its `background_color()`.
...
During each cycle of the event loop, when a winit window event is received, it is translated to a Vizia `WindowEvent` and added to an event queue in the `Context`. At the end of each cycle, a `MainEventsCleared` event is received, which is where Vizia processes changes to the application.
