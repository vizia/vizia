...
/// ## Custom Switch
///
/// The `text` modifier combined with a `map` on the lens can be used to customize the icon used by the Switch.
///
/// ```
/// # use vizia_core::prelude::*;
/// #
...
impl Switch {
    ...
    pub fn new(cx: &mut Context, checked: impl Lens<Target = bool>) -> Handle<Self> {
        ...
        Self { on_toggle: None }
            .build(cx, |cx| {
                HStack::new(cx, |cx| {
                    HStack::new(cx, |_|{}).class("switch-handle").hoverable(false);
                }).class("switch-handle-container").hoverable(false);
            })
