...
fn draw_view(cx: &mut DrawContext, canvas: &mut Canvas) {
    ...
    if cx.cosmic_context.has_buffer(cx.current) {
        println!("We have a buffer: {}", cx.current);
        cx.cosmic_context.sync_styles(cx.current, &cx.style, &cx.cache);
        if let Ok(draw_commands) = cx.cosmic_context.fill_to_cmds(canvas, cx.current) {
            println!("Draw commands: {:?}", draw_commands.alpha_glyphs);
            println!("Draw commands: {:?}", draw_commands.color_glyphs);
