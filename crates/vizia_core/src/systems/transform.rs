...
pub(crate) fn transform_system(cx: &mut Context) {
    ...
    for entity in iter {
        ...
        if let Some(parent) = cx.tree.get_layout_parent(entity) {
            ...
            // let parent_clip_bounds =
            //     cx.cache.clip_bounds.get(parent).copied().unwrap_or(root_bounds);
            // let intersection = clip_bounds.intersection(&parent_clip_bounds);

            // if let Some(stored_clip_bounds) = cx.cache.clip_bounds.get_mut(entity) {
            //     *stored_clip_bounds = intersection;
            // } else {
            //     cx.cache.clip_bounds.insert(entity, intersection);
            // }
