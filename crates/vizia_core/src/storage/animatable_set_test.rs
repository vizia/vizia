...
pub(crate) struct SharedIndex {
    // Either an index to shared data or a hash of a variable name
    data_index: u32,
    animation: Animation,
}
...
pub(crate) struct AnimatableSet2<T: Interpolator> {
    ...
    pub(crate) variable_data: SparseSet<Variable>,
    ...
    T: 'static + Default + Clone + Interpolator + PartialEq + std::fmt::Debug,
{
    ...
    /// Playing an animation for a particular entity will clone the animation state to the
    ...
    pub(crate) fn link(
        ...
        for rule in rules.iter() {
            if let Some(shared_data_index) = self.shared_data.dense_idx(*rule) {
                ...
                let data_index = self.inline_data.sparse[entity_index].data_index;
                // Already linked
                if !data_index.is_inline() && data_index.index() == rule.index() {
                    return false;
                }
