...
fn backspace_offset(text: &impl EditableText, mut cursor: usize) -> usize {
    #[derive(PartialEq)]
    enum State {
        Start,
        Lf,
    ...
    while state != State::Finished && cursor > 0 {
        let code_point = text.prev_codepoint(cursor).unwrap_or('0');
        ...
        match state {
            ...
            State::BeforeZwj => {
                if is_emoji(code_point) {
                    ...
                    state = if is_emoji_modifier(code_point) {
                        State::BeforeEmojiModifier
                    } else {
                        State::BeforeEmoji
                    };
    ...
    for _ in 0..delete_code_point_count {
        if let Some(offset) = text.prev_codepoint_offset(cursor) {
            cursor = offset;
        }
    }
...
fn is_emoji(c: char) -> bool {
    is_in_asc_list(c, &EMOJI_TABLE, 0, EMOJI_TABLE.len() - 1)
}
