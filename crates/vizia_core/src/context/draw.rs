...
use skia_safe::{
    BlurStyle, ClipOp, FilterMode, IRect, MaskFilter, Matrix, Paint, PaintStyle, Path,
    PathDirection, Point, RRect, Rect, Shader, TileMode,
};
                ...
                match image {
                    ...
                    ImageOrGradient::Image(image_name) => {
                        if let Some(image) = self.resource_manager.images.get(image_name) {
                            ...
                            // let pic = skia_safe::Image::from_encoded(unsafe {
                            //     skia_safe::Data::new_bytes(image.image.as_bytes())
                            // });
                            // if let Some(pic) = pic {
                            let mut paint = Paint::default();
                            println!("draw image: {} {}", width, height);
                            paint.set_anti_alias(true);
