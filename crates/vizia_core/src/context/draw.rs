...
use skia_safe::{
    BlurStyle, ClipOp, MaskFilter, Paint, PaintStyle, Path, PathDirection, Point, RRect, Rect,
    Shader, TileMode,
};
...
impl<'a> DrawContext<'a> {
    ...
    pub fn clip_region(&self) -> Option<skia_safe::Path> {
        let bounds = self.bounds();
    ...
    pub fn draw_text(&mut self, canvas: &Canvas) {
        if let Some(paragraph) = self.text_context.text_paragraphs.get(self.current) {
            ...
            //         bounds.x + padding_left + paragraph.max_width(),
            //         bounds.y + padding_top + paragraph.height(),
            //     ),
            //     &paint,
            // );
