...
use vizia_style::{alignment, LengthPercentageOrAuto};
...
impl DrawContext<'_> {
    ...
    pub fn draw_text(&mut self, canvas: &Canvas) {
        if let Some(paragraph) = self.text_context.text_paragraphs.get(self.current) {
            ...
            let (mut top, mut left) = match alignment {
            ...
            top *= bounds.height() - paragraph.height();
            left *= bounds.width() - paragraph.max_width();

            let mut padding_top = match self.padding_top() {
            ...
            // if let Units::Stretch(val) = self.padding_top() {
            ...
            let padding_right = match self.padding_right() {
                Units::Pixels(val) => val,
                _ => 0.0,
            };
            ...
            // if let Units::Stretch(val) = self.padding_left() {
            //     padding_left = (horizontal_free_space * val / horizontal_flex_sum).round()
            // }

            paragraph.paint(
                canvas,
                ((bounds.x + padding_left + left).round(), (bounds.y + padding_top + top).round()),
            );
        }
    }
}
