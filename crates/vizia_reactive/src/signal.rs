...
impl<T, S> SignalTrack<T> for Signal<T, S> {
    fn id(&self) -> Id {
        self.id
    }
}
...
impl<T, S> fmt::Debug for Signal<T, S> {
...
impl<T, S> Signal<T, S> {
    ...
    pub fn read_only(&self) -> ReadSignal<T, S> {
        ReadSignal { id: self.id, ty: PhantomData, st: PhantomData }
    }

    /// Create a Setter of this Signal
    pub fn write_only(&self) -> WriteSignal<T, S> {
        WriteSignal { id: self.id, ty: PhantomData, st: PhantomData }
    }
}

impl<T: Send + Sync + 'static> Signal<T, SyncStorage> {
    ...
    pub fn new_sync(value: T) -> Self {
        ...
        Signal { id, ty: PhantomData, st: PhantomData }
    }
...
impl<T: 'static> Signal<T, UnsyncStorage> {
    #[cfg_attr(debug_assertions, track_caller)]
...
impl<T: Clone + Send + Sync> SignalGet<T> for Signal<T, SyncStorage> {
    fn id(&self) -> Id {
        self.id
    }
}

impl<T: Send + Sync> SignalWith<T> for Signal<T, SyncStorage> {
    fn id(&self) -> Id {
        self.id
    }
}

impl<T: Send + Sync> SignalRead<T> for Signal<T, SyncStorage> {
    fn id(&self) -> Id {
        self.id
    }
...
impl<T: Send + Sync> SignalUpdate<T> for Signal<T, SyncStorage> {
    fn id(&self) -> Id {
        self.id
    }
...
impl<T: Send + Sync> SignalWrite<T> for Signal<T, SyncStorage> {
    fn id(&self) -> Id {
        self.id
    }
...
impl<T: Clone> SignalGet<T> for Signal<T, UnsyncStorage> {
    fn id(&self) -> Id {
        self.id
    }
}

impl<T> SignalWith<T> for Signal<T, UnsyncStorage> {
    fn id(&self) -> Id {
        self.id
    }
}

impl<T> SignalRead<T> for Signal<T, UnsyncStorage> {
    fn id(&self) -> Id {
        self.id
    }
...
impl<T> SignalUpdate<T> for Signal<T, UnsyncStorage> {
    fn id(&self) -> Id {
        self.id
    }
...
impl<T> SignalWrite<T> for Signal<T, UnsyncStorage> {
    fn id(&self) -> Id {
        self.id
    }
