...
impl Id {
    ...
    pub(crate) fn set_scope(&self) {
        RUNTIME.with(|runtime| {
            ...
            runtime.children.borrow_mut().entry(scope).or_default().insert(*self);
...
mod tests {
    ...
    use crate::{
        Effect, Signal, SignalTrack, SignalUpdate,
        runtime::{RUNTIME, Runtime},
        scope::Scope,
    };
    ...
    fn effect_disposed_when_dependency_signal_disposed() {
        ...
        let (signal, setter) = signal_scope.create_split_signal(0);
    ...
    fn signals_created_by_effect_are_disposed_with_effect() {
        ...
        let (dep_signal, dep_setter) = dep_scope.create_split_signal(0);
        ...
        parent.enter(|| {
            ...
            Effect::new(move |_| {
                ...
                if created_signal.borrow().is_none() {
                    created_signal.replace(Some(Signal::new(0)));
                }
            });
        });
    ...
    fn disposing_scope_drops_signals_and_effects() {
        let scope = Scope::new();
        let (signal, setter) = scope.create_split_signal(0);
    ...
    fn set_parent_disposes_with_new_parent() {
        ...
        let signal = child.create_signal(42);
        let signal_id = signal.id();
    ...
    fn set_parent_removes_from_old_parent() {
        ...
        RUNTIME.with(|runtime| {
            let children = runtime.children.borrow();
            let new_children =
                children.get(&new_parent.0).expect("new parent should have children");
