...
impl Offset {
    ...
    pub fn center() -> Offset {
        Offset { x: HorizontalPosition::Center, y: VerticalPosition::Center }
    }
...
impl<L1, L2> From<(L1, L2)> for Offset
where
...
impl<'i> Parse<'i> for Offset {
    fn parse<'t>(input: &mut Parser<'i, 't>) -> Result<Self, ParseError<'i, CustomParseError<'i>>> {
        if let Ok(x) = input.try_parse(HorizontalPosition::parse) {
            // Try parsing a vertical offset next.
            if let Ok(y) = input.try_parse(VerticalPosition::parse) {
                return Ok(Offset { x, y });
            }

            // If it didn't work, assume the first actually represents a y offset,
            // and the next is an x offset. e.g. `center left` rather than `left center`.
            let x =
            ...
            // Try parsing a horizontal offset next.
            if let Ok(x) = input.try_parse(HorizontalPosition::parse) {
                return Ok(Offset { x, y });
            } else {
                return Ok(Offset { x: HorizontalPosition::Center, y: VerticalPosition::Center });
            }
        } else {
